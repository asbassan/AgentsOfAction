...
The `/testsql` skill provides a command-line interface for managing a SQL Server 2022 Docker container for YourProject testing and development. It handles setup, shutdown, backup, restore, and status monitoring with intelligent workflows.
...
Found 2 existing backup(s):
  - testdb_20260126_143022.bak (125M)
  - testdb_20260125_091534.bak (118M)
...
Backup name: testdb_20260126_151045

10 percent processed.
20 percent processed.
...
100 percent processed.
Processed 12800 pages for database 'testdb', file 'testdb_Data' on file 1.
...
Backup location:
  Container: /var/opt/mssql/backup/testdb_20260126_151045.bak
  Host: .claude/testSQLSetup/backups/testdb_20260126_151045.bak
  Size: 125M
...
Found 3 existing backup(s):
  - testdb_20260126_151045.bak (125M)
  - testdb_20260126_143022.bak (125M)
  - testdb_20260125_091534.bak (118M)
...
**Backup Details**:
- Backup name: `testdb_YYYYMMDD_HHMMSS.bak`
...
Backup name: testdb_20260126_152317
...
Backup location:
  Container: /var/opt/mssql/backup/testdb_20260126_152317.bak
  Host: .claude/testSQLSetup/backups/testdb_20260126_152317.bak
  Size: 125M
```
...
Select backup to restore:
  1) testdb_20260126_151045.bak (125M)
  2) testdb_20260126_143022.bak (125M)
  3) testdb_20260125_091534.bak (118M)

Enter number (or press Enter for most recent): 1

Restoring from: testdb_20260126_151045.bak
...
```
testdb_YYYYMMDD_HHMMSS.bak
```

Examples:
- `testdb_20260126_143022.bak` → Jan 26, 2026 at 2:30:22 PM
- `testdb_20260125_091534.bak` → Jan 25, 2026 at 9:15:34 AM
...
cp .claude/testSQLSetup/backups/testdb_20260126_143022.bak ~/my-backups/

# Delete old backups (older than 30 days)
find .claude/testSQLSetup/backups/ -name "testdb*.bak" -mtime +30 -delete
```
...
rm testdb_2026*.bak  # Delete old backups
...
docker cp my-script.sql sqlserver-dev:/tmp/

# Execute script
docker exec -it sqlserver-dev /opt/mssql-tools/bin/sqlcmd \
  -S localhost -U sa -P 'YourSecurePassword123!' -d testdb -i /tmp/my-script.sql
...
1. **Kill active connections**
   ...
   RESTORE DATABASE testdb FROM DISK='/var/opt/mssql/backup/backup.bak' WITH REPLACE;
...
3. **Check backup integrity**
   ```sql
   RESTORE VERIFYONLY FROM DISK='/var/opt/mssql/backup/testdb_20260126.bak';
   ```
...
# Then execute:
docker exec -it sqlserver-dev /opt/mssql-tools/bin/sqlcmd \
  -S localhost -U sa -P 'YourSecurePassword123!' -d testdb -i /scripts/my-script.sql
```
...
# Copy to host
docker cp sqlserver-dev:/tmp/export.csv ./export.csv
```
...
docker exec -it sqlserver-dev /opt/mssql-tools/bin/sqlcmd \
  -S localhost -U sa -P 'YourSecurePassword123!' \
  -Q "SELECT COUNT(*) AS Connections FROM sys.dm_exec_sessions WHERE database_id = DB_ID('testdb');"
```
...
docker cp src/DAMS-Scripts/script.sql sqlserver-dev:/tmp/
