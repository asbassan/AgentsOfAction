# YourProject SQL Server Docker Test Environment

## Overview

This Docker setup provides a fully-configured SQL Server 2022 Developer Edition instance for YourProject testing and development. The container is configured with 6GB RAM, 100GB storage capacity, and full DBA capabilities.
...
docker exec -it sqlserver-dev /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P 'YourSecurePassword123!'
```
...
**Via sqlcmd:**
```bash
docker exec -it sqlserver-dev /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P 'YourSecurePassword123!' -Q "BACKUP DATABASE [testdb] TO DISK='/var/opt/mssql/backup/testdb.bak' WITH COMPRESSION;"
```
...
```bash
docker exec -it sqlserver-dev /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P 'YourSecurePassword123!' -i /path/to/script.sql
```
...
# Execute script
docker exec -it sqlserver-dev /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P 'YourSecurePassword123!' -i /tmp/script.sql
```
...
Then execute:
```bash
docker exec -it sqlserver-dev /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P 'YourSecurePassword123!' -i /scripts/your_script.sql
```
...
```bash
docker exec -it sqlserver-dev /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P 'YourSecurePassword123!' -Q "SELECT @@VERSION"
```
...
# In container
docker exec -it sqlserver-dev df -h
```
...
docker exec -it sqlserver-dev /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P 'YourSecurePassword123!' -Q "BACKUP DATABASE [testdb] TO DISK='/var/opt/mssql/backup/testdb_before_removal.bak'"

# Copy to host
docker cp sqlserver-dev:/var/opt/mssql/backup ./backups/
```
...
5. **Regular backups:**
   ...
   docker exec sqlserver-dev /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P 'YourSecurePassword123!' -Q "BACKUP DATABASE [testdb] TO DISK='/var/opt/mssql/backup/testdb_$(date +%Y%m%d_%H%M%S).bak' WITH COMPRESSION"
   ```
...
### Connection String for YourProject Scripts

```csharp
// C# connection string
var connectionString = "Server=localhost,1433;Database=testdb;User Id=sa;Password=YourSecurePassword123!;TrustServerCertificate=True;";
```

### Running YourProject Scripts

```bash
# Copy YourProject scripts to container
docker cp src/YourProject-Scripts sqlserver-dev:/scripts/
...
```bash
# Mount YourProject-Scripts directory for validation
docker run --rm \
  -v "$(pwd)/src/YourProject-Scripts:/scripts" \
...
docker cp .claude/testSQLSetup/14514176/Test_DropUnusedIndexDMV.sql sqlserver-dev:/tmp/
...
- **YourProject Documentation**: See `README.md` in repo root
- **Script Validator**: See `src/ScriptValidator/README.md`
- **SQL Script Guidelines**: See `src/YourProject-Scripts/ProductionInvestigationScripts/pull_request_template.md`
...
This setup is for internal Microsoft use with CAP-YourProject development and testing.
...
**Created**: 2026-01-26
**Version**: 1.0
**Maintained by**: CAP-YourProject Team
