...
The `/testsql` skill is an **agent-based skill** (no bash script, Claude executes it) that manages a Docker-based SQL Server 2022 test environment for YourProject development.
...
### Example: YourProject Test Schema

```sql
-- .claude/testSQL/bootstrap/project-test.sql
CREATE DATABASE ProjectTest;
GO
USE ProjectTest;
GO

-- Simulate OrganizationBase
...
PRINT 'YourProject test schema created';
```

**Usage:**
```bash
/testsql setup project-test.sql
...
var connStr = "Server=localhost,1433;Database=testdb;User Id=sa;Password=YourSecurePassword123!;TrustServerCertificate=True;";
...
```bash
sqlcmd -S localhost,1433 -U sa -P YourSecurePassword123! -d testdb -Q "SELECT * FROM Customers"
```
...
docker cp src/Scripts/script.sql sqlserver-dev:/tmp/

# Run script
docker exec -it sqlserver-dev /opt/mssql-tools/bin/sqlcmd \
  -S localhost -U sa -P 'YourSecurePassword123!' -d ProjectTest \
  -i /tmp/script.sql
...
4. Test connection:
   ```bash
   docker exec sqlserver-dev /opt/mssql-tools/bin/sqlcmd \
     -S localhost -U sa -P 'YourSecurePassword123!' -Q "SELECT 1"
   ```
