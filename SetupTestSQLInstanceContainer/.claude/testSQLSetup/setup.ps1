# =============================================
# DAMS SQL Server Container Setup Script (PowerShell)
# =============================================

Write-Host "==========================================" -ForegroundColor Cyan
Write-Host "DAMS SQL Server Docker Setup" -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "  - Container Name: $($CONTAINER_NAME ?? 'dams-sqlserver-dev')"
...
$containerExists = docker ps -a --format '{{.Names}}' | Select-String -Pattern "^$($CONTAINER_NAME ?? 'dams-sqlserver-dev')$"

if ($containerExists) {
    Write-Host "WARNING: Container '$($CONTAINER_NAME ?? 'dams-sqlserver-dev')' already exists" -ForegroundColor Yellow
...
while ($retryCount -lt $maxRetries) {
    $healthStatus = docker inspect --format='{{.State.Health.Status}}' $($CONTAINER_NAME ?? 'dams-sqlserver-dev') 2>$null
...
Write-Host "  docker exec -it $($CONTAINER_NAME ?? 'dams-sqlserver-dev') /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P '$SA_PASSWORD'"
Write-Host ""
Write-Host "==========================================" -ForegroundColor Cyan
